...
class Task
{
    ...
    public function __construct(string $label, ?string $description, bool $completed, ?int $assignee, ?int $id = null, ?string $status = null)
    {
        $this->id = $id;
        $this->status = $status;
        $this->label = $label;
    ...
    public static function getAll(): array
    {
        ...
        if ($result === false) { throw new Exception('Could not retrieve the tasks from the database.', 500);
            ...
            while ($object = $result->fetch_object()) {
                $task = new self($object->label, $object->description, (bool)$object->completed, $object->assignee, (int)$object->id, $object->status);
                $rows[] = $task;
            }
        }
