...
class Task
{
    ...
    public static function getAll(): array
    {
        $db = Database::get();
        $tasks = $db->fetchAll(
            'SELECT * FROM tasks', [], ['foreach' => function ($row) {
    ...
    {
        $db = Database::get();
        list($rowCount, $insertedId, $error) = $db->query(
            <<<'SQL'
...
SQL
    ...
    function update(?string $label, ?string $description, ?string $status, ?int $assignee): Task
    {
        ...
        if (empty($changes)) {
            throw new Exception('No changes were provided.', 400);
        }

        $sql = "UPDATE `tasks` SET " . implode(', ', $changes) . " WHERE `id` = :id";
        $params['id'] = $this->id;

        list($rowCount, $insertedId, $error) = $db->query($sql, $params);

        if ($error) {
            throw new Exception('The task could not be updated in the database.', 500);
        }

        // Read from DB to get the updated task
